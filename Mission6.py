...
class ScatterPlotApp:
    ...
    def setup_ui(self):
        ...
        self.x_combo = ttk.Combobox(control_frame, textvariable=self.x_var, values=self.numeric_cols)
        ...
        self.y_combo = ttk.Combobox(control_frame, textvariable=self.y_var, values=self.numeric_cols)
    ...
    def update_plot(self):
        ...
        if not x_col or not y_col:
            messagebox.showwarning("알림", "X축과 Y축을 모두 선택해주세요.")
            return
        ...
        try:
            if show_reg:
                # 회귀선 그리기 (데이터가 숫자가 아니면 여기서 에러 발생)
            ...
            self.ax.set_title(f"[{x_col}] 과 [{y_col}]의 관계")
            self.fig.tight_layout()
            self.canvas.draw()

        except Exception as e:
            messagebox.showerror("데이터 오류", f"선택한 열로 그래프를 그릴 수 없습니다.\n숫자 데이터인지 확인하세요.\n(에러: {e})")
